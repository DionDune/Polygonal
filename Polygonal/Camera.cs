    ...
    public class Camera
    {
        ...
        public void renderWorld(SpriteBatch spriteBatch, GraphicsDevice GraphicsDevice, Settings settings, Screen Screen, List<Object> Objects)
        {
            ...
            for (int i = objectListOrdered.Count() - 1; i >= 0; i--)
            {
                ...
                if (settings.cameraRenderFaces)
                    ...
                    {
                        Game1.DrawTriangleEffect(Game1._basicEffect, GraphicsDevice, new Vector3(Face.Vertex1.X, Face.Vertex1.Y, 0),
                                                                                        new Vector3(Face.Vertex2.X, Face.Vertex2.Y, 0),
                                                                                        new Vector3(Face.Vertex3.X, Face.Vertex3.Y, 0), Face.Color);

                        if (settings.cameraRenderPolygonEdges)
                        {
                            Game1.DrawLineEffect(Game1._basicEffect, GraphicsDevice, new Vector3(Face.Vertex1.X, Face.Vertex1.Y, 0),
                                                                                        new Vector3(Face.Vertex2.X, Face.Vertex2.Y, 0),
                                                                                        Color.Black);
                            Game1.DrawLineEffect(Game1._basicEffect, GraphicsDevice, new Vector3(Face.Vertex1.X, Face.Vertex1.Y, 0),
                                                                                        new Vector3(Face.Vertex3.X, Face.Vertex3.Y, 0),
                                                                                        Color.Black);
                            Game1.DrawLineEffect(Game1._basicEffect, GraphicsDevice, new Vector3(Face.Vertex2.X, Face.Vertex2.Y, 0),
                                                                                        new Vector3(Face.Vertex3.X, Face.Vertex3.Y, 0),
                                                                                        Color.Black);
                        }
                    }
        ...
        private List<ObjectFace> GetObjectFaces(Screen Screen, Object Object)
        {
            ...
            foreach (ObjectFace Face in Object.Faces)
            {
                ...
                {
                    (Face.Vertex1 * Object.Scale) + Object.Position,
                    (Face.Vertex2 * Object.Scale) + Object.Position,
                    (Face.Vertex3 * Object.Scale) + Object.Position
                };
                ...
                foreach (Vector3 Vert in FaceVerts)
                {
                    ...
                    //Test 
                    DifferenceZ = Vert.Z - WorldPosition.Z;
                    //WorldAngleY = (float)Math.Atan2(DifferenceZ, DistanceX) * (float)(180 / Math.PI);


                    float ReletiveAngleY;
                    float AngleOffsetY = (Direction.Y - (FOV.Y / 2));
                    ReletiveAngleY = WorldAngleY - AngleOffsetY;
